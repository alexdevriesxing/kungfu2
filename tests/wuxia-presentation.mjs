...
if(mode==='static'){
  ...
  for(const token of ['THEMES','decorateAllStages','drawBossIntro','drawVictorySeal','drawBrushTransition','drawRicePaperGrain','chapterForMode'])if(!source.includes(token))fail(`Wuxia presentation feature missing: ${token}`);
...
api.drawPresentation(game);
game.mode='act-two-complete';game.wuxiaFx.lastMode='explore';api.drawPresentation(game);
check('presentation remains safe on chapter screens',true);
